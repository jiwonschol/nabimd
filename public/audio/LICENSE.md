# Audio licensing

The following shipped sound-effect files were obtained from Mixkit:

- `matched.mp3`
- `summary.mp3`
- `try-again.mp3`

They are governed by the Mixkit Free License for Sound Effects and the Mixkit
User Terms:

- [Mixkit License](https://mixkit.co/license/)
- [Mixkit Free Sound Effects](https://mixkit.co/free-sound-effects/)
- [Mixkit User Terms](https://mixkit.co/terms/)

The sounds may be incorporated into personal and commercial projects.
Attribution is appreciated but not required.

Do not redistribute, license, sell, or offer the sounds as standalone assets,
and do not aggregate them into a stock collection, inventory, asset library,
or similar or competing service.

These audio files are not licensed under this repository's AGPL-3.0-or-later
code license or CC BY-SA 4.0 content license, and they are not CC0. The Mixkit
license and terms above govern them.

## Source originals

The repository also stores the unconverted `.wav` originals of the Mixkit
sounds above — `src/sound/matched.wav`, `src/sound/summary.wav`, and
`src/sound/tryagain.wav`. The same Mixkit license and terms govern them.
