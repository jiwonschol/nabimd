# Batch 047: Level 2 and Level 3 unlock candidates

Prepare short, independently reviewable exercises for every Level 2 and Level 3
syntax that the current parser, grader, guided checkpoint generator, and learner
copy can teach. Do not include heading IDs because the current GFM parser does
not support them.

## Coverage

- Create five dedicated standard problems for each of the five supported Level 2
  elements and each of the five render-observable Level 3 elements. Do not author
  angle-bracket email problems: GFM renders their plaintext starters as the same
  links, so adding the brackets would not produce a visible learning task. Level 3 targets must
  be short, coherent, human-readable documents rather than isolated snippets.
- Create five standard mixed exercises owned by Level 3. Each must combine two
  Level 3 elements, produce no more than five guided checkpoints, and avoid a
  separated repeat of the same checkpoint term.
- Keep every target within its curriculum owner's runtime line budget.

## Grading and learner contract

- Grade the actual Markdown syntax shape with the real parser-backed
  `syntax-presence` check. Do not grade exact prose, spelling, or case.
- Every dedicated problem must have canonical, alternate-prose, case/spelling,
  missing, malformed, and matched fixture roles. Every match check must have a
  direct failing fixture.
- Every target must produce at least one real guided checkpoint whose learner
  term is already recorded by the generated syntax capability matrix.
- Keep five content variants in every retry family so the first five turns do
  not repeat a card.

## Publication boundary

Freeze deterministic candidates, fixtures, parser contract, verification, and
the review manifest only. Do not add reviewer verdicts, editorial evidence, or
publish this batch. Two independent reviewers and a separate editorial actor
must seal the final manifest before publication.
