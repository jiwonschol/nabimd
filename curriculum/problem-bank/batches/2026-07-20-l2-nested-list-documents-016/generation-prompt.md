...
- Require exactly one top-level list containing exactly one descendant list. Require at least two visibly meaningful items in both lists; HTML comments, definitions, dividers, breaks, control/default-ignorable characters, and whitespace do not count as visible item content.
...
- Use only deterministic CommonMark AST engine checks. Do not add GFM, HTML-authored answers, runtime AI, network calls, exact-string matching, or vocabulary crawling.
