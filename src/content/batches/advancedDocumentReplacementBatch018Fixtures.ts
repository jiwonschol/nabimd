...
import {
  advancedDocumentReplacementBatch018Inputs,
  advancedDocumentReplacementBatch018Problems,
} from "./advancedDocumentReplacementBatch018Problems"
...
function transformScope(
  ...
  if (scope.kind === "block") {
    throw new Error("Batch 018 fixtures do not author block-scoped checks")
  }
...
function varyCaseAndSpelling(source: string, index: number): string {
  return source.replace(/^# ([^\n]+)/, (_match, title: string) => {
    ...
    words[wordIndex] =
      word.length > 4 ? `${word.slice(0, -1).toLowerCase()}x` : word.toLowerCase()
    return `# ${words.join(" ")}`
  })
}
...
function withQuotedOpeningInsteadOfParagraph(source: string): string {
  return source.replace(
    /^(# .+\n\n)([\s\S]*?)(\n\n## )/,
...
function withEditorialReview(
  source: string,
  family: (typeof advancedDocumentReplacementBatch018Inputs)[number]["family"],
): { source: string; reviewId: string } {
  if (family === "level5-refactor") {
    return {
      source: source.replace("**Changed:**", "**Changed:** **Note:**"),
      reviewId: "keep-report-labels-focused",
    }
  }
  return {
  ...
  switch (check.kind) {
    ...
    case "block-count":
      if (check.block === "heading" && check.depth === 2) {
        return `${problem.target}\n\n## Extra section\n\nThis extra section changes the required anatomy.`
      }
      ...
      if (check.block === "thematic-break") {
        return transformScope(problem.target, check.scope, (section) =>
          section.replace(/^---$/m, "Evidence boundary removed."),
        )
      }
...
function expectedFeedbackForMutation(
  ...
  if (
    ...
    (check.kind === "block-count" &&
      (check.block === "heading" || check.block === "thematic-break"))
  ) {
...
function materializeFixtures(
  problem: (typeof advancedDocumentReplacementBatch018Problems)[number],
  index: number,
): ProblemFixture[] {
  const input = advancedDocumentReplacementBatch018Inputs[index]!
  const familyProblems = advancedDocumentReplacementBatch018Problems.filter(
    (_, candidateIndex) =>
      advancedDocumentReplacementBatch018Inputs[candidateIndex]!.family ===
      input.family,
  )
...
export const advancedDocumentReplacementBatch018Fixtures: readonly ProblemFixture[] =
  advancedDocumentReplacementBatch018Problems.flatMap(materializeFixtures)
