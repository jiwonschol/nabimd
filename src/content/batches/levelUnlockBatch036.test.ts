...
import { levelUnlockBatch036Fixtures } from "./levelUnlockBatch036Fixtures"
import { levelUnlockBatch036Problems } from "./levelUnlockBatch036Problems"

const levelTwo = curriculumLevels.find((entry) => entry.id === "level-2")!
const levelThree = curriculumLevels.find((entry) => entry.id === "level-3")!
const batchDirectory = `${process.cwd()}/curriculum/problem-bank/batches/2026-08-31-l2-l3-unlock-036`

describe("Level 2 and 3 unlock batch 036", () => {
  ...
  it("freezes failure fixtures for parser and renderer edge cases", () => {
    const unreferencedDefinitions = levelUnlockBatch036Fixtures.filter(
      (fixture) => fixture.id?.endsWith("-unreferenced-definition"),
    )
    const literalCodeBackslashes = levelUnlockBatch036Fixtures.filter(
      (fixture) => fixture.id?.endsWith("-literal-code-backslashes"),
    )
    const mailtoUris = levelUnlockBatch036Fixtures.filter(
    ...
    for (const fixture of mailtoUris) {
      const problem = levelUnlockBatch036Problems.find(
  ...
  it("passes the source schema and fixture coverage gate", () => {
    expect(
      validateProblemBank(levelUnlockBatch036Problems, levelUnlockBatch036Fixtures),
    ).toEqual([])
    expect(levelUnlockBatch036Fixtures).toHaveLength(24 * 6 + 14 + 4 + 3)
  })
  it("provides two dedicated retry variants for every supported element", () => {
    const singles = levelUnlockBatch036Problems.filter(
  ...
  it("grades every canonical target and gives it a guided checkpoint within budget", () => {
    for (const problem of levelUnlockBatch036Problems) {
  ...
  it("adds two eligible Level 3-owned mixed exercises", () => {
    const mixed = levelUnlockBatch036Problems.filter(
      (problem) => getCurriculumElements(problem).length > 1,
    )
  ...
  it("makes both entries available when the frozen candidates are published", () => {
    const projected = [...problemBank, ...levelUnlockBatch036Problems]
