...
function integrationContractChecks(problemId: string): MatchCheck[] {
  return [
    ...
    visibleListCheck(
      ...
      3,
      30,
      "Use at least three visible bullets in the contract-surface section.",
    ),
    {
    ...
    visibleListCheck(
      ...
      3,
      60,
      "Use at least three visible numbered implementation steps.",
    ),
    visibleListCheck(
      ...
      3,
      70,
      "Use at least three visible acceptance bullets.",
    ),
    {
...
function evidenceRecoveryChecks(problemId: string): MatchCheck[] {
  return [
    ...
    visibleListCheck(`${problemId}-authority`, h2Section(1), true, 3, 40, "Use at least three visible ordered authority sources."),
    ...stageChecks(problemId, 2, 3, 50),
    visibleListCheck(`${problemId}-rollback`, h2Section(3), true, 3, 60, "Use at least three visible numbered rollback steps."),
    visibleListCheck(`${problemId}-constraints`, h2Section(4), false, 3, 70, "Use at least three visible hard-constraint bullets."),
    visibleListCheck(`${problemId}-stops`, h2Section(5), false, 3, 80, "Use at least three visible stop-condition bullets."),
    codeCheck(problemId, "verification", 6, 90),
...
function boundedRefactorChecks(problemId: string): MatchCheck[] {
  return [
    ...
    visibleListCheck(`${problemId}-reading`, h2Section(0), true, 3, 30, "Use at least three visible ordered authority sources."),
    visibleListCheck(`${problemId}-scope`, h2Section(1), false, 3, 40, "Use at least three visible allowed-scope bullets."),
    {
    ...
    visibleListCheck(`${problemId}-forbidden`, h2Section(2), false, 3, 50, "Use at least three visible forbidden-shortcut bullets."),
    ...stageChecks(problemId, 3, 3, 60),
    codeCheck(problemId, "regression", 4, 70),
    visibleListCheck(`${problemId}-stops`, h2Section(5), false, 3, 80, "Use at least three visible stop-condition bullets."),
    codeCheck(problemId, "report", 6, 90),
  ]
}
...
function coordinatedRolloutChecks(problemId: string): MatchCheck[] {
  return [
    ...
    visibleListCheck(`${problemId}-authority`, h2Section(1), true, 3, 40, "Use at least three visible ordered authority sources."),
    ...stageChecks(problemId, 2, 3, 50),
    visibleListCheck(`${problemId}-compatibility`, h2Section(3), false, 3, 60, "Use at least three visible compatibility-guard bullets."),
    visibleListCheck(`${problemId}-rollback`, h2Section(4), true, 3, 70, "Use at least three visible numbered rollback steps."),
    {
    ...
    visibleListCheck(`${problemId}-verification`, h2Section(6), false, 3, 90, "Use at least three visible verification bullets."),
    {
      ...
      requireNonemptyContent: true,
      priority: 95,
      feedback: "Use at least three nonempty inline-code items in the verification ledger.",
    },
    codeCheck(problemId, "report", 6, 100),
  ]
}
...
const familyTeaching: Record<AdvancedDocumentFamily, ProblemInput["teaching"]> = {
  ...
  "evidence-recovery-work-order": {
    concept: "A compact recovery order separates evidence, authority, recovery stages, rollback, constraints, stops, proof, and reporting.",
    howTo: "Use three-item authority, rollback, constraint, and stop lists, three H3 stages, and fenced verification and report blocks.",
    example: "# Recovery order\n\nState the mission.\n\n## Evidence boundary\n\n> Record one observed failure.\n\n---\n\n## Authority order\n\n1. Rules\n2. Policy\n3. Current code\n\n## Recovery stages\n\n### Contain\n\n- Limit impact\n- Preserve evidence\n\n### Repair\n\n- Make one change\n- Run focused checks\n\n### Prove\n\n- Run the full gate\n- Record results",
  },
  "bounded-refactor-work-order": {
    concept: "A compact refactor order separates authority, scope, forbidden shortcuts, execution, regression, stops, and reporting.",
    howTo: "Use three-item reading, scope, and prohibition lists, three H3 stages, a regression command block, and a report template.",
...
const familyMetadata: Record<AdvancedDocumentFamily, {
  ...
  "evidence-recovery-work-order": {
    ...
    skillIds: ["evidence-boundary", "authority-order", "recovery-stages", "rollback-contract", "stop-conditions", "verification-contract", "final-report"],
    syntaxTokens: ["#", "##", "###", ">", "---", "1.", "-", "```bash", "```markdown"],
    hints: ["Separate evidence and authority before writing the recovery stages.", "Use exactly three ### stages and three short items in each later list.", "Finish with fenced command evidence and a fenced final-report template."],
  },
  "bounded-refactor-work-order": {
    level: 5,
    ...
    familyId: "agent-ready-work-order",
    profile: "agent-workflow",
    skillIds: ["authority-order", "scope-boundary", "forbidden-shortcuts", "three-stage-execution", "regression-guards", "stop-conditions", "final-report"],
    syntaxTokens: ["#", "##", "###", "1.", "-", "`", "```bash", "```markdown"],
    hints: ["Put the reading order before the allowed and forbidden scope lists.", "Use exactly three ### stages and keep every list to three short items.", "Finish with regression commands, stop conditions, and a report template."],
  },
  "coordinated-rollout-work-order": {
    level: 5,
    ...
    hints: ["Map the provider and consumers with one compact nested list before authority.", "Use exactly three ### phases, then add three compatibility and rollback items.", "Finish with a quoted pause rule and a verification section containing inline code and a report template."],
  },
}
...
const integrationSources: readonly IntegrationSpecSource[] = [
  {
    ...
    surface: ["Accept one `SupportEvent` payload", "Preserve the provider event identifier", "Return one documented acknowledgment"],
    ...
    implementation: ["Freeze valid and invalid payload fixtures", "Verify the signature at the adapter boundary", "Record the stable identifier before dispatch"],
    acceptance: ["A valid new event is processed once", "A repeated identifier produces no duplicate update", "An invalid signature changes no stored data"],
  ...
  },
  {
    ...
    surface: ["Accept one `ContactRow` schema", "Preserve the source row number", "Return imported and rejected counts"],
    ...
    implementation: ["Parse the header and source row numbers", "Validate each row before mapping fields", "Write valid contacts in one bounded operation"],
    acceptance: ["Valid rows retain their source order", "Invalid rows are reported without partial records", "Repeated identifiers do not create duplicates"],
  ...
  },
  {
    ...
    surface: ["Accept one `DigestWindow` value", "Select only eligible updates", "Reuse one delivery identifier"],
    ...
    implementation: ["Freeze the delivery-window calculation", "Select updates through the existing repository", "Claim the stable delivery identifier"],
    acceptance: ["One window produces at most one digest", "An empty update set produces no message", "A temporary failure can be retried safely"],
  ...
  },
  {
    ...
    surface: ["Accept one `ArchiveRequest`", "Preserve the requesting account identifier", "Produce one manifest and checksum"],
    ...
    implementation: ["Authorize the request before reading records", "Build the manifest from approved fields", "Create and verify the archive checksum"],
    acceptance: ["Unauthorized requests create no archive", "The manifest lists every included file", "A checksum mismatch exposes no download"],
...
const recoverySources: readonly RecoverySource[] = [
  {
    ...
    authority: ["Read AGENTS.md for repository rules", "Read the queue safety policy", "Read ADR-018 for delivery idempotency"],
    ...
    constraints: ["No customer record is deleted to make counts agree", "One delivery identifier creates at most one notification", "Logs contain no notification body or credential"],
    ...
    commands: ["npm test --workspace apps/worker -- duplicate-delivery", "npm run typecheck"],
  },
  {
    ...
    authority: ["Read AGENTS.md for repository rules", "Read the search recovery policy", "Read ADR-024 for index ownership"],
    ...
    constraints: ["The database remains the source of truth", "No missing result is replaced with invented content", "The rebuild does not alter primary records"],
    ...
    commands: ["npm test --workspace apps/search -- consistency", "npm run benchmark -- index-compare"],
  },
]
...
${source.stages.map((stage) => `### ${stage.title}\n\n${bulletLines(stage.actions)}`).join("\n\n")}
...
const refactorSources: readonly RefactorSource[] = [
  {
    ...
    reading: ["Read AGENTS.md", "Read the date display decision record", "Inspect shared formatting tests"],
    allowed: ["Edit `packages/formatting/src/date.ts`", "Edit `packages/formatting/src/date.test.ts`", "Update approved web and email callers"],
    forbidden: ["Do not change stored timestamps", "Do not replace locale-aware formatting with fixed strings", "Do not update snapshots before reviewing the rendered difference"],
    stages: [{ title: "Stage 1 — pin current behavior", actions: ["Add boundary fixtures for locale and time zone", "Run the focused formatting suite"] }, { title: "Stage 2 — create the shared helper", actions: ["Move one verified behavior at a time", "Keep the public helper signature narrow"] }, { title: "Stage 3 — migrate callers", actions: ["Update web and email consumers", "Remove only proven duplicate helpers"] }],
    commands: ["npm test --workspace packages/formatting", "npm run typecheck"],
  ...
  },
  {
    ...
    reading: ["Read AGENTS.md and the privacy policy", "Read ADR-031 for the approved event client", "Inspect the event contract fixtures"],
    allowed: ["Edit `packages/analytics/src/adapter.ts`", "Edit `packages/analytics/src/adapter.test.ts`", "Update approved callers behind the adapter"],
    forbidden: ["Do not rename public events", "Do not add raw customer text to payloads", "Do not bypass consent to satisfy a fixture"],
    stages: [{ title: "Stage 1 — freeze the event contract", actions: ["Add deterministic allowlist fixtures", "Record consent and unavailable-client behavior"] }, { title: "Stage 2 — add the approved client", actions: ["Keep vendor code behind the adapter", "Map only allowlisted payload fields"] }, { title: "Stage 3 — migrate callers", actions: ["Update web and worker imports", "Remove verified legacy adapter paths"] }],
    commands: ["npm test --workspace packages/analytics", "npm run typecheck"],
...
const rolloutSources: readonly RolloutSource[] = [
  {
    ...
    mission: "Roll out one versioned response across the fictional Harbor Tasks API, web client, and mobile client while preserving compatibility.",
    systemMap: [{ name: "API provider", details: ["Versioned schema", "Compatibility serializer"] }, { name: "Web consumer" }, { name: "Mobile consumer" }],
    authority: ["Read AGENTS.md in each repository", "Read the shared response contract", "Read the approved rollout decision"],
    ...
    compatibility: ["Existing clients continue to read the current version", "Unknown optional fields remain safe", "Cached mobile data remains readable"],
    rollback: ["Restore the prior provider serializer", "Restore the previous typed clients", "Run the complete compatibility suite"],
    ...
    verification: ["Run `npm test --workspace apps/api -- contract`", "Run `npm test --workspace apps/web -- project-response`", "Run `npm test --workspace apps/mobile -- project-response`"],
  },
  {
    ...
    mission: "Roll out one notification preference schema across the fictional Willow Mail API, worker, and admin console without changing saved choices.",
    systemMap: [{ name: "API schema owner", details: ["Preference contract", "Compatibility mapping"] }, { name: "Delivery worker" }, { name: "Admin console" }],
    authority: ["Read repository-wide agent rules", "Read the preference data policy", "Read ADR-027 for schema ownership"],
    ...
    compatibility: ["Existing preference choices retain their meaning", "Missing optional values use the documented fallback", "The worker reads only the API-owned contract"],
    rollback: ["Restore compatibility mapping in the API", "Restore the previous worker reader", "Run preference and delivery journeys"],
    ...
    verification: ["Run `npm test --workspace apps/api -- preferences`", "Run `npm test --workspace apps/worker -- delivery-choice`", "Run `npm test --workspace apps/admin -- preference-form`"],
  },
]
