...
import { levelUnlockBatch043Problems } from "../../src/content/batches/levelUnlockBatch043Problems"
...
import {
  buildLevelUnlockBatch043Artifacts,
  buildLevelUnlockBatch043Publication,
  checkLevelUnlockBatch043State,
  readCommittedLevelUnlockBatch043,
  writeLevelUnlockBatch043Artifacts,
} from "./levelUnlockBatch043Support"
...
function expectReviewPhase({ editorial, state, publication }: ReviewPhase) {
  if (editorial === null) {
    ...
    expect(publication.errors).toContain(
      "Batch 2026-08-31-l2-l3-unlock-043 requires separate editorial evidence",
    )
    return
  }
...
describe("schema-v2 Level 2 and 3 unlock batch 043", () => {
  it("normalizes all candidates and verifies every real-engine fixture", () => {
    expect(computed.normalized.candidateCount).toBe(55)
    expect(computed.fixtureArtifact.fixtures).toHaveLength(levelUnlockBatch043Fixtures.length)
  ...
  it("binds all candidates to the real syntax detector", () => {
    ...
    expect(computed.manifest.entries).toHaveLength(levelUnlockBatch043Problems.length)
  })
  ...
  it("tracks the empty review directory without forging review JSON", async () => {
    const reviewReadme = await readFile(
      `${repositoryRoot}/curriculum/problem-bank/batches/2026-08-31-l2-l3-unlock-043/reviews/README.md`,
      "utf8",
    )
  ...
  it("keeps committed mechanical evidence deterministic in every review phase", async () => {
    const committed = await readCommittedLevelUnlockBatch043({ repositoryRoot })
    const state = checkLevelUnlockBatch043State({ computed, committed })
    expect(
  ...
  it("keeps publication closed until every independent seal exists", async () => {
    const committed = await readCommittedLevelUnlockBatch043({ repositoryRoot })
    const state = checkLevelUnlockBatch043State({ computed, committed })
    const publication = buildLevelUnlockBatch043Publication({ computed, committed })
    expectReviewPhase({
  ...
  it("refuses to rewrite evidence after review begins", async () => {
    ...
    if (committed.reviews.length === 0 && committed.editorial === null) {
      await expect(
        writeLevelUnlockBatch043Artifacts({ repositoryRoot, computed }),
