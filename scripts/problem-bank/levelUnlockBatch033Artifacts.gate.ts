...
import { levelUnlockBatch033Problems } from "../../src/content/batches/levelUnlockBatch033Problems"
import {
  buildLevelUnlockBatch033Artifacts,
  buildLevelUnlockBatch033Publication,
  checkLevelUnlockBatch033State,
  readCommittedLevelUnlockBatch033,
  writeLevelUnlockBatch033Artifacts,
} from "./levelUnlockBatch033Support"

const repositoryRoot = process.cwd()
const computed = await buildLevelUnlockBatch033Artifacts({ repositoryRoot })

describe("schema-v2 Level 2 and 3 unlock batch 033", () => {
  it("normalizes all candidates and verifies every real-engine fixture", () => {
    expect(computed.normalized.candidateCount).toBe(24)
    expect(computed.fixtureArtifact.fixtures).toHaveLength(levelUnlockBatch033Fixtures.length)
  ...
  it("binds all candidates to the real syntax detector", () => {
    ...
    expect(computed.manifest.entries).toHaveLength(levelUnlockBatch033Problems.length)
  })

  it("tracks the empty review directory without forging review JSON", async () => {
    const reviewReadme = await readFile(
      `${repositoryRoot}/curriculum/problem-bank/batches/2026-08-30-l2-l3-unlock-033/reviews/README.md`,
      "utf8",
    )
  ...
  it("keeps committed mechanical evidence deterministic and unsealed", async () => {
    const committed = await readCommittedLevelUnlockBatch033({ repositoryRoot })
    expect(checkLevelUnlockBatch033State({ computed, committed })).toEqual({
  ...
  it("refuses publication until independent seals exist", async () => {
    const committed = await readCommittedLevelUnlockBatch033({ repositoryRoot })
    const publication = buildLevelUnlockBatch033Publication({ computed, committed })
    expect(publication.errors).toContain("Batch 2026-08-30-l2-l3-unlock-033 requires 2 independent reviews")
    expect(publication.errors).toContain("Batch 2026-08-30-l2-l3-unlock-033 requires separate editorial evidence")
  })

  it("refuses to rewrite evidence after review begins", async () => {
    const committed = await readCommittedLevelUnlockBatch033({ repositoryRoot })
    expect(committed.reviews).toEqual([])
    await expect(writeLevelUnlockBatch033Artifacts({ repositoryRoot, computed })).resolves.toBeUndefined()
  })
})
