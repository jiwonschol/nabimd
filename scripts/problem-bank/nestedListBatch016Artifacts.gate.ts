...
import {
  buildNestedListBatch016Artifacts,
  buildNestedListBatch016Publication,
  checkNestedListBatch016State,
  publishNestedListBatch016Artifacts,
  readCommittedNestedListBatch016,
  writeNestedListBatch016Artifacts,
} from "./nestedListBatch016Support"

const repositoryRoot = process.cwd()
const computed = await buildNestedListBatch016Artifacts({ repositoryRoot })

describe("schema-v2 Level 2 nested-list document batch 016", () => {
  it("runs every candidate fixture through the real learner engine", () => {
    ...
    expect(computed.fixtureArtifact.fixtures).toHaveLength(
      nestedListBatch016Fixtures.length,
    )
  ...
  it("preserves the 320-problem bank before unanimous acceptance", () => {
    expect(computed.priorTracker.acceptedTotal).toBe(320)
  ...
  it("keeps committed mechanical evidence deterministic", async () => {
    const committed = await readCommittedNestedListBatch016({ repositoryRoot })
    const state = checkNestedListBatch016State({ computed, committed })
    expect(
  ...
  it("refuses to rewrite evidence after review begins", async () => {
    ...
    if (committed.reviews.length === 0 && committed.editorial === null) {
      expect(checkNestedListBatch016State({ computed, committed })).toEqual({
    ...
    await expect(
      writeNestedListBatch016Artifacts({ repositoryRoot, computed }),
    ).rejects.toThrow("immutable after review or editorial evidence exists")
  })

  it("publishes all 12 only after two independent reviews and editorial acceptance", async () => {
    const committed = await readCommittedNestedListBatch016({ repositoryRoot })
    const state = checkNestedListBatch016State({ computed, committed })
    if (committed.editorial === null) {
      ...
      expect(committed.tracker.acceptedTotal).toBe(320)
      expect(committed.summary).toBeNull()
      return
    }

    const publication = buildNestedListBatch016Publication({
      computed,
      committed,
    })
    expect(publication.errors).toEqual([])
    expect(publication.tracker.acceptedTotal).toBe(332)
    ...
    expect(publication.tracker.counts.byFamily).toMatchObject({
      "rebuild-nested-list-documents": 12,
    })
  ...
  it("keeps publication fail-closed while editorial evidence is absent", async () => {
    ...
    await expect(
      publishNestedListBatch016Artifacts({ repositoryRoot, computed }),
