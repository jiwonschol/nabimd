...
- **D17 — Reproduction answers begin with prose (expanded 2026-07-21).** Every answer sheet is pre-filled with the Goal's learner-visible words, blank lines, and line breaks, with Markdown marks removed; the learner restores the structure instead of retyping or reorganizing prose. Saved drafts still win. The seed is never graded or auto-corrected, and it is derived at the runtime projection boundary so accepted problem-bank evidence remains immutable.
