...
- **D12 — Exercise frame becomes a CBT desk (confirmed 2026-07-19, Level 5 example revised 2026-07-21).** Replace the editor-like Goal/Source/Live Preview/Coach layout with a familiar test frame: one fixed top bar and two equal sheets, immutable rendered Goal on the left and the learner's answer on the right. The answer sheet owns `Write` and `Preview`; after a failed Check, `Preview` becomes `Review`. Hint reveals vertically inside Goal. Longer compact developer documents scroll inside the equal panels, never through the page chrome; full work orders are no longer the Level 5 destination.
- **D13 — Problem-bank variety is vocabulary-led (confirmed 2026-07-19, curriculum context revised 2026-07-21).** Issue #9 expands the number of vetted prompts without changing the grammar-only grading policy. Level 1 uses short familiar nouns and phrases from fruit, weather, and learning tools. Higher levels combine skills around realistic human-readable documents; Level 5 uses developer and AI-assisted-work vocabulary in short examples. Repetition practices a Markdown skill with new content instead of asking the learner to copy the same prose again.
- **D14 — Definitive level identities (revised 2026-07-21).** The five entry, header, and completion labels are: `Level 1 — Learn the syntax`, `Level 2 — Rebuild real documents`, `Level 3 — Write for people`, `Level 4 — Write for work`, and `Level 5 — Write for developers`. The current UI retains the former Level 4–5 labels until its separately reviewed change lands.
- **D15 — Fixed Goal at every level (revised 2026-07-21).** Every level shows the authored Goal as the fixed target. Higher-level prompts may name the Markdown anatomy to practice, but they do not ask the learner to author new prose. Every level is graded by the same Markdown-structure predicates; prose is never compared.
...
- **D17 — Reproduction answers begin with prose (target expanded 2026-07-21).** The approved target is for every answer sheet to be pre-filled with the Goal's learner-visible words, blank lines, and line breaks, with Markdown marks removed; the learner restores the structure instead of retyping or reorganizing prose. Saved drafts still win. The seed is never graded or auto-corrected, and it is derived at the runtime projection boundary so accepted problem-bank evidence remains immutable. The current `main` runtime hydrates Levels 1–2 only; Levels 3–5 join this contract when their separately reviewed Practice change lands.
