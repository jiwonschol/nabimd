...
- **D17 — Reproduction answers begin with prose (confirmed 2026-07-20).** Level 1–2 answer sheets are pre-filled with the rendered target's learner-visible text and line breaks, with Markdown structure removed; the learner adds the marks instead of retyping the prose. Saved drafts still win. Level 3–5 remain blank composition exercises. The seed is never graded or auto-corrected, and it is derived at the runtime projection boundary so accepted problem-bank evidence remains immutable.
