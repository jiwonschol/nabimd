...
test("completes a run and reveals full documents only from Summary", async ({
  ...
  await expect(
    page.getByRole("region", { name: "Rendered document" }),
  ).toBeVisible()
  await expect(
  ...
  await expect(page.locator(".verdict-notice--matched")).toHaveCount(0)
  await expect(page.getByLabel("Run summary")).not.toHaveClass(
    /run-summary--waiting/,
  )
  await expect
    .poll(() =>
